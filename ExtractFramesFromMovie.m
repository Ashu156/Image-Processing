...
movieFullFileName = fullfile('C:\ashu-hp-backup\Ashu-hp-backup\desktop\Desktop\Handbrake\A_U_02.05.15.mp4');
...
try
	...
	for frame = 1:videoObject.NumberOfFrames
...
% 		title('Binarized Difference Image', 'FontSize', fontSize);
	...
	for frame = 1 :3: numberOfFrames - 1
		% Construct an output image file name.
		outputBaseFileName = sprintf('Frame %4.4d.png', frame);
		outputFullFileName = fullfile(outputFolder, outputBaseFileName);
		% Read the image in from disk.
		thisFrame = imread(outputFullFileName);
        thisFrame = imresize(thisFrame,[231, 452]); % Resizing the frames
